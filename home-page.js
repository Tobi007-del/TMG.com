...
let preloaderTextContainer = document.getElementById(
  "preloader-text-container",
);

const skipWaitBtn = document.querySelector("#skip-wait-btn");
const preload = () => {
...
function preloader() {
  ...
  let third = document.getElementById("third-text");
  let fourth = document.getElementById("fourth-text");
  let fifth = document.getElementById("fifth-text");
...
function skipWait() {
  document.querySelector("#skip-wait-btn").addEventListener("click", () => {
    page();
    return () => {
      document
        .querySelector("#skip-wait-btn")
        .removeEventListener("click", page());
    };
  });
}
...
function link() {
  let allLinks = document.querySelectorAll(
    ".moving-picture-link, .panels a, .search-lines a, .hidden-r-panel-main a",
  );
  ...
  allLinks.forEach((allLink) => {
    allLink.addEventListener("click", (event) => {
      ...
      alert(
        "The site is still in developement so no movies are available at the moment, we apologize!!! but you can preview some movie trailers at the top of the home page",
      );
    });
  });
}
...
function GardenerNameFunction() {
  if (
    localStorage.greetGardener === undefined ||
    localStorage.greetGardener === "undefined"
  ) {
...
document.getElementById("demo").addEventListener("click", () => {
  ...
  if (
    personChange.toUpperCase() === "YES" ||
    personChange.toUpperCase() === "Y"
  ) {
    ...
    if (newPerson.length <= maxCharacters) {
      if (newPerson === "" || newPerson === null) {
        ...
        if (
          ...
          document.getElementById("user-image").src =
            "/TMG.com/SPARE-PICS/person_one.jpg";
          localStorage.picsrc = "/TMG.com/SPARE-PICS/person_one.jpg";
        }
      } else {
      ...
      if (
        dlightItems.length < 2 &&
        localStorage.greetGardener === UNIQUE_TEXT &&
        (localStorage.picsrc === UNIQUE_IMAGE_D ||
          localStorage.picsrc === UNIQUE_IMAGE_L ||
          localStorage.picsrc === "/TMG.com/SPARE-PICS/person.png" ||
          localStorage.picsrc === "/TMG.com/SPARE-PICS/person_one.jpg" ||
    ...
    personChange.toUpperCase() === "NO" ||
    personChange.toUpperCase() === "N"
  ) {
  ...
  if (Notification?.permission === "granted") {
    ...
    Notification.requestPermission().then((status) => {
      if (status === "granted") {
        ...
        alert(
          "You have to give notification permission to get the welcome notification",
        );
      }
    });
  } else {
...
function toggleBallSG(ball, value) {
  ...
  if (value === 1) {
    setTimeout(() => {
      document.querySelector(ball).classList.add("scala");
    }, 0);
    setTimeout(() => {
      document.querySelector(ball).classList.remove("scala");
    }, 250);
    ...
    setTimeout(() => {
      document.querySelector(ball).classList.add("scale");
    }, 0);
    setTimeout(() => {
      document.querySelector(ball).classList.remove("scale");
    }, 250);
...
function movingPicture() {
  ...
  const movingVideoContainers = document.getElementsByClassName(
  ...
  const watchTrailerButtons = document.querySelectorAll(
  ...
  const movingPictureImageCoversLeft = document.getElementsByClassName(
    "moving-picture-image-cover-left",
  );
  const movingPictureImageCoversRight = document.getElementsByClassName(
    "moving-picture-image-cover-right",
  );
  ...
  function reversePlayTrailer(x) {
    ...
    if (
      movingPictures[x].computedStyleMap().get("transform")[0].x.value == -4700
    ) {
  ...
  function movingFrame() {
    ...
    movingPictures.forEach((movingPicture, r) => {
      ...
      if (
        movingPicture.computedStyleMap().get("transform")[0].x.value == -4700
      ) {
